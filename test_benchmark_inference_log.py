...
with torch.no_grad():
    ...
    if args.perf:
        ...
        for j in range(2):
            ...
            num_tokens += f" -- Generating {gen_tokens} tokens, {ids.shape[-1]} token prompt..|"
    ...
    try:
        with connection.cursor() as cursor:
            tokens_sec = tokens_sec[:-1]
            mdl = os.path.basename(args.model).replace(".safetensors", "")
            _params = ' '.join(sys.argv[1:])

            # split this into 2 inserts to make the data useful in mysql:
            ts = tokens_sec.split("|")
            ns = num_tokens.split("|")

            # get run_id:
            sql = "SELECT max(run_id) as 'run_id' FROM llm_benchmark"
            cursor.execute(sql)
            result = cursor.fetchone()
            if result['run_id']:
                run_id = int(result['run_id'])+1
            else:
                run_id = 1

            for x in range(0,2):
                #INSERT INTO llm_benchmark (ADD ACT ORDER, GROUPSIZE, MODEL_SIZE to DB)
                sql = "INSERT INTO llm_benchmark (model, context, tokens_sec, perplexity, run_id, run_params) VALUES (%s, %s, %s, %s, %s, %s)"
                values = (mdl, ns[x], ts[x], pplx, run_id, _params)
